...
- **Automated Package & Services Deployment**: Verifies and installs **45+ system, font, and GUI packages** (including `hyprland`, `waybar`, `dunst`, `rofi-wayland`, `sddm`, `kitty`, `firefox`, `visual-studio-code-bin`, `dolphin`, `ananicy-cpp`, `antigravity`, etc.) and automatically enables core system services (`bluetooth`, `NetworkManager`, `sddm`, and the `ananicy-cpp` auto-nice daemon for performance optimization).
